...
class PlanFlanUser(HttpUser):
    ...
    def browse_with_city_filter(self):
        ...
        with self.client.get(
            f"/liste_etablissements?ville={city}", catch_response=True
        ) as response:
            if response.status_code != 200:
                response.failure(
                    f"Échec chargement avec filtre ville {city}: {response.status_code}"
                )
            else:
    ...
    def test_api_endpoints(self):
        ...
        test_city = random.choice(self.available_cities)[
            :3
        ...
        with self.client.get(
            "/api/etablissements?format=json&ville=Paris", catch_response=True
        ) as response:
            if response.status_code != 200:
            ...
            if "etablissement/" in content:
                ...
                with self.client.get(
                    ...
                    if response.status_code == 404:
                        ...
                        response.failure(
                            f"Échec chargement établissement {test_id}: {response.status_code}"
                        )
        except:

...
def test_no_real_database_used():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "mysql+pymysql://test:test@localhost/test_db"}):
        ...
        assert (
            "sqlite" in TestConfig.SQLALCHEMY_DATABASE_URI.lower()
        ), "TestConfig doit utiliser SQLite pour éviter la vraie base de données"
        assert (
            ":memory:" in TestConfig.SQLALCHEMY_DATABASE_URI
        ...
        assert (
            ConfigProd.SQLALCHEMY_DATABASE_URI != TestConfig.SQLALCHEMY_DATABASE_URI
        ), "ConfigProd doit utiliser une base de données différente de TestConfig"
        assert (
            "mysql" in ConfigProd.SQLALCHEMY_DATABASE_URI.lower()
            or "postgresql" in ConfigProd.SQLALCHEMY_DATABASE_URI.lower()
        ...
        assert (
            TestConfig.UPLOAD_FOLDER is not None
...
def test_upload_folder_consistency():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "mysql+pymysql://test:test@localhost/test_db"}):
        ...
        assert (
            ConfigProd.UPLOAD_FOLDER == expected_path
...
def test_upload_folder_permissions(temp_upload_env):
    ...
    assert os.path.exists(upload_folder), "Le dossier uploads doit exister"
...
def test_photos_route_uses_correct_config():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "mysql+pymysql://test:test@localhost/test_db"}):
        ...
        with app.app_context():
            ...
            assert (
                app.config["UPLOAD_FOLDER"] == TestConfig.UPLOAD_FOLDER
            ), "La configuration doit avoir le bon UPLOAD_FOLDER"

            assert (
                app.config["ALLOWED_EXTENSIONS"] == TestConfig.ALLOWED_EXTENSIONS
            ), "La configuration doit avoir les bonnes extensions autorisées"

            # Vérifier que la taille maximale est correcte
            assert (
                app.config["MAX_CONTENT_LENGTH"] == TestConfig.MAX_CONTENT_LENGTH
            ...
            assert (
                TestConfig.ALLOWED_EXTENSIONS == ConfigProd.ALLOWED_EXTENSIONS
            ), "Les extensions autorisées doivent être cohérentes entre test et production"
            assert (
                TestConfig.MAX_CONTENT_LENGTH == ConfigProd.MAX_CONTENT_LENGTH
...
def test_dockerfile_upload_folder_creation(temp_upload_env):
    ...
    assert docker_upload_folder.endswith(
        expected_suffix
...
def test_config_values_match_dockerfile():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "mysql+pymysql://test:test@localhost/test_db"}):
        ...
        assert (
            ConfigProd.UPLOAD_FOLDER == "/app/static/uploads"
        ), "UPLOAD_FOLDER doit être /app/static/uploads comme dans le Dockerfile"

        assert ConfigProd.ALLOWED_EXTENSIONS == {
            "png",
            "jpg",
            "jpeg",
            "gif",
        ...
        assert (
            ConfigProd.MAX_CONTENT_LENGTH == 16 * 1024 * 1024
        ), "La taille maximale doit être 16 Mo comme configuré"

        # Vérifier que la configuration de test a des valeurs cohérentes
        assert (
            TestConfig.UPLOAD_FOLDER is not None and len(TestConfig.UPLOAD_FOLDER) > 0
        ), "La configuration de test doit avoir un UPLOAD_FOLDER valide"
...
def test_config_constants_defined():
    ...
    with patch.dict(os.environ, {"DATABASE_URL": "mysql+pymysql://test:test@localhost/test_db"}):
        ...
        for attr in required_attrs:
            assert hasattr(
                ConfigProd, attr
            ), f"La constante {attr} doit être définie dans ConfigProd"
            assert hasattr(
                TestConfig, attr
            ), f"La constante {attr} doit être définie dans TestConfig"

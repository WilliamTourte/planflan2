...
def utilisateur_test(test_client):
    ...
    user = Utilisateur(
        pseudo="testuser", email="test@example.com", password="testpassword", is_admin=False
    )
    db.session.add(user)
    db.session.commit()
    return user
...
def test_verifier_etablissement_existant(test_client, etablissement_existant):
    ...
    response = test_client.post(
        "/verifier_etablissement", json={"nom": "Boulangerie Test"}, content_type="application/json"
    )
...
def test_verifier_etablissement_sans_nom(test_client):
    ...
    response = test_client.post("/verifier_etablissement", json={}, content_type="application/json")

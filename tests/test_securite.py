...
@pytest.mark.auth
def dummy_auth():
    """Marqueur pour les tests d'authentification"""
    pass


@pytest.mark.admin
def dummy_admin():
    """Marqueur pour les tests d'administration"""
    pass


@pytest.mark.slow
def dummy_slow():
    """Marqueur pour les tests lents"""
    pass


@pytest.mark.critical
def dummy_critical():
    """Marqueur pour les tests critiques"""
    pass
...
def setup_full_data(app):
    """Crée des données de test minimales pour les tests de sécurité."""
    with app.app_context():
...
@pytest.fixture
def setup_full_data(app):
    """Crée des données de test complètes pour les tests de sécurité."""
    with app.app_context():
        from flask_bcrypt import Bcrypt
...
def test_acces_route_protegee_sans_connexion(client, setup_full_data):
...
@pytest.mark.auth
def test_connexion_utilisateur_valide(client, setup_full_data):
...
def test_connexion_utilisateur_invalide(client):
    ...
    response = client.post(
        "/login",
        data={"pseudo": "invalide", "password": "motdepasseinvalide"},
        follow_redirects=True,
    )
    ...
    assert (
        b"invalides" in response.data
        or b"invalid" in response.data
        or b"credentials" in response.data
    )


def test_deconnexion_utilisateur(client, setup_full_data):
...
def test_utilisateur_regular_acces_route_admin(client, setup_full_data):
...
def test_utilisateur_regular_modification_ressource_autre_utilisateur(client, setup_full_data):
    ...
    with client.application.app_context():
        ...
        response = client.post(
            f"/modifier_flan/{flan_admin.id_flan}",
            data={
...
def test_admin_acces_routes_admin(client, setup_full_data):
    """Test qu'un admin peut accéder aux routes admin."""
    # Se connecter en tant qu'admin
    client.post(
        "/login",
        data={"pseudo": "admin", "password": "adminpassword"},
        follow_redirects=True,
    )
...
def test_admin_modification_ressource_autre_utilisateur(client, setup_full_data):
    ...
    with client.application.app_context():
        ...
        response = client.post(
            f"/modifier_flan/{flan_user.id_flan}",
            data={
        ...
        assert (
            b"succes" in response.data.lower()
            or b"success" in response.data.lower()
            or b"mis a jour" in response.data.lower()
        )
...
def test_protection_csrf_activer(client, setup_full_data):
...
def test_soumission_formulaire_sans_token_csrf(client, setup_full_data):
    ...
    with client.application.app_context():
        ...
        try:
            ...
            response = client.post(
                "/dashboard",
                data={"profile-pseudo": "nouveau_pseudo"},
                follow_redirects=True,
            )
...
def test_injection_sql_dans_recherche(client, setup_full_data):
...
def test_injection_html_dans_formulaire(client, setup_full_data):
...
def test_caracteres_speciaux_dans_entrees(client, setup_full_data):
...
def test_mot_de_passe_hache(client, setup_full_data):
...
def test_mot_de_passe_trop_court(client):
    ...
    assert (
        b"au moins 6" in response.data
        or b"6 characters" in response.data
        or b"too short" in response.data
    )
...
def test_mot_de_passe_trop_long(client):
    ...
    assert (
        b"succes" in response.data.lower()
        or b"success" in response.data.lower()
        or b"compte" in response.data.lower()
    )
...
def test_deconnexion_automatique_apres_inactivite(client, setup_full_data):
...
def test_session_utilisateur_isolee(client, setup_full_data):
...
def test_acces_api_sans_authentification(client, setup_full_data):
    ...
    assert response.status_code in [
        200,
        500,
    ]  # 200 pour succès, 500 pour erreur serveur
    # Vérifier que la réponse est en JSON
    if response.status_code == 200:
        assert response.content_type == "application/json"


def test_acces_api_avec_authentification(client, setup_full_data):
...
def test_upload_fichier_type_invalide(client, setup_full_data):
    ...
    assert response.status_code in [
        ...
        404,
        405,
    ]  # 404 si la route n'existe pas, 405 si méthode non autorisée


def test_upload_fichier_taille_excessive(client, setup_full_data):

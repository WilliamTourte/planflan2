...
def test_geoloc_route_responds(client_with_csrf):
    ...
    response = client_with_csrf.post('/geoloc', json={"latitude": 45.764043, "longitude": 4.835659})
...
def test_etablissements_proches_route_responds(client_with_csrf):
    ...
    # Tester avec des données valides
    response = client_with_csrf.post('/etablissements_proches', 
...
def test_extraire_infos_adresse_route_responds(client_with_csrf):
    ...
    # Tester avec des données valides
    response = client_with_csrf.post('/extraire_infos_adresse', 
                                    json={"adresse": "1 rue de Test, 69001 Lyon"})
    assert response.status_code == 200
    data = response.get_json()
...
def test_verifier_etablissement_route_responds(client_with_csrf):
    ...
    # Tester avec des données valides
    response = client_with_csrf.post('/verifier_etablissement', 
                                    json={"nom": "Test Etablissement"})
    assert response.status_code == 200
    data = response.get_json()
...
def test_upload_route_responds(client_with_csrf):
    ...
    response = client_with_csrf.post('/upload', 
                                    data={},
...
def test_supprimer_compte_route_requires_login(client_with_csrf):
    ...
    response = client_with_csrf.post('/supprimer_compte', 

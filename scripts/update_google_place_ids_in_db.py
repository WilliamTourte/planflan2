...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('update_place_ids_db.log'),
...
def mettre_a_jour_google_place_ids(fichier_json):
    ...
    with app.app_context():
        try:
            ...
            logger.info(f"📊 Nombre d'établissements dans le fichier : {len(data.get('features', []))}")
            ...
            for feature in data["features"]:
                try:
                    ...
                    logger.error(f"❌ Erreur pour {feature.get('properties', {}).get('location', {}).get('name', 'inconnu')}: {e}")

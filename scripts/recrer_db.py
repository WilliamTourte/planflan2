...
def mettre_a_jour_visite_label():
    ...
    logger.warning(
        f"ATTENTION : {len(etablissement_records)} établissements ont été mis à jour avec des valeurs aléatoires"
    )
...
if __name__ == "__main__":
    ...
    confirmation = input(
        "ATTENTION : Cette operation va supprimer et recreer les donnees. Voulez-vous continuer ? (oui/non) : "
    )
...
with app.app_context():
    ...
    from app.models import Utilisateur, StatutModeration
    ...
    if os.path.exists(chemin_lieux_json):
        ...
        if len(sys.argv) > 1:
            ...
            print(
                "Pour importer des lieux, utilisez : python recrer_db.py chemin/vers/fichier.json"
            )
            logger.info(
                "Pour importer des lieux, utilisez : python recrer_db.py chemin/vers/fichier.json"
            )

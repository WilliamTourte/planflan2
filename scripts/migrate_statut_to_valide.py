    ...
    try:
        ...
        with app.app_context():
            ...
            if etab_en_attente > 0:
                ...
                db.session.query(Etablissement).filter_by(statut="EN_ATTENTE").update(
                    {"statut": "VALIDE"}
                )
            ...
            if eval_en_attente > 0:
                ...
                db.session.query(Evaluation).filter_by(statut="EN_ATTENTE").update(
                    {"statut": "VALIDE"}
                )
...
def main():
    ...
    response = input(
        "Cette opération mettra à jour tous les statuts EN_ATTENTE vers VALIDE.\n"
